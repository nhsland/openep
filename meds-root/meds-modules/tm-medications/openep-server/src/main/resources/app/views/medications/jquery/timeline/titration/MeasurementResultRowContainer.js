Class.define('app.views.medications.timeline.titration.MeasurementResultRowContainer', 'tm.jquery.Container', {
  ...
  _buildGui: function()
  {
    ...
    var titrationType = titrationData.getTitrationType();
    ...
    var iconContainer = new tm.jquery.Container({
      ...
      width: 48,
      height: 48,
      cls: "icon-container " + this._getTitrationTypeIcon(titrationType),
    ...
    if (titrationType)
    {
      therapyTitrationDescription += '<p class="TextDataBold">' +
          this.getView().getDictionary("TitrationType." + titrationType) + '</p>';
    }

    if (titrationData.getUnit())
    {
      therapyTitrationDescription += '<div class="circle">' + '</div>' + " " + '<span class="TextData">'
          + tm.jquery.Utils.escapeHtml(titrationData.getUnit()) + '</span>';
    }
    ...
    {
      var tickInterval = app.views.medications.MedicationTimingUtils.getTitrationTickIntervalByType(titrationType);
      self._chartHelpers
          .createChart(component.getDom(), 200, tickInterval, false, true)
  ...
  _applyChartData: function(chart)
  {
    ...
    var maxQuantity = measurementResults.reduce(
        function(maxQuantity, measurementResult)
        {
    ...
    var minQuantity = measurementResults.reduce(
        function(minQuantity, measurementResult)
        {
          var resultQuantity = measurementResult.getQuantity();
  ...
  _getTitrationTypeIcon: function(titrationType)
  {
    ...
    else if (titrationType === enums.MAP)
    {
      return "icon_map";
    }
    ...
    else if (titrationType === enums.APTTR)
    {
      return "icon_apttr";
    }
