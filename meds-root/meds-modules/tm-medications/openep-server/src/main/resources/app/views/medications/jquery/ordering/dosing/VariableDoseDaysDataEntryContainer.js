Class.define('app.views.medications.ordering.VariableDoseDaysDataEntryContainer', 'app.views.common.containers.AppDataEntryContainer', {
  ...
  _buildComponents: function()
  {
    ...
    this.referenceDosePane = new app.views.medications.ordering.dosing.DoseContainer({
      orderingBehaviour: this.getOrderingBehaviour(),
      view: view,
    ...
    this.unitLabel = app.views.medications.MedicationUtils.crateLabel(
  ...
  _buildGui: function()
  {
    ...
    firstRowTitleContainer.add(app.views.medications.MedicationUtils.crateLabel('TextLabel', view.getDictionary('fill.empty.cells'), "5 0 0 0"));
    ...
    this.add(app.views.medications.MedicationUtils.crateLabel('TextLabel', view.getDictionary('unit')));
    this.add(this.unitLabel);
    this.add(app.views.medications.MedicationUtils.crateLabel('TextLabel', view.getDictionary('dose')));
  ...
  _showProtocolRepeatLabel: function(date)
  {
    ...
    {
      this._repeatInfoLabel.setHtml(view.getDictionary("protocol.repeat.until.cancelled"));
    }
  ...
  _addThreeDotsContainer: function()
  {
    ...
    threeDots.setTooltip(app.views.medications.MedicationUtils.createTooltip(view.getDictionary("last.dose.repeating"), "bottom", view));
    ...
    var protocolOptionsDialog = appFactory.createDataEntryDialog(
        ...
        function(resultData)
        {
          if (resultData)
          {
            ...
            if (resultData.value.repeatWholeProtocolTimes > 0)
            {
  ...
  {
    var columnsToAdd = this.dayLabels.length * (times + 1) - this.dayLabels.length;
  ...
  _fixDates: function()
  {
    ...
    for (var i = 0; i < this.dateLabels.length; i++)
    {
      ...
      var dateDisplay = app.views.medications.MedicationTimingUtils.getDateWithoutYearDisplay(this.getView(), applicationDate);
  ...
  {
    var dateDisplay = app.views.medications.MedicationTimingUtils.getDateWithoutYearDisplay(this.getView(), date);
  ...
  _buildDosePane: function(numerator, rowIndex, columnIndex)
  {
    ...
    return new app.views.medications.ordering.dosing.DoseContainer({
      orderingBehaviour: this.getOrderingBehaviour(),
      referenceData: this.getReferenceData(),
  ...
  _presentData: function()
  {
    ...
    for (var j = 0; j < this.timedDoseElements.length; j++)
    {
      ...
      if (app.views.medications.MedicationUtils.getIndexOfHourMinute(time, times) === -1)
      {
        times.push(time);
      }
    }
    ...
    for (var m = 0; m < this.timedDoseElements.length; m++)
    {
      ...
      var row = app.views.medications.MedicationUtils.getIndexOfHourMinute(timedDoseElement.doseTime, times);
  ...
  _getNumberOfRowsFromDosingFrequency: function()
  {
    ...
    var administrationTimes =
        app.views.medications.MedicationTimingUtils.getFrequencyTimingPattern(
