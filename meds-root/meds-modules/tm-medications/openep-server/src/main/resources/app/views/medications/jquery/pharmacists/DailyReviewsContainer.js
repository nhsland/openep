Class.define('app.views.medications.pharmacists.DailyReviewsContainer', 'tm.jquery.Container', {
  statics: {
  ...
  Constructor: function (config)
  {
    ...
    this.registerEventTypes('app.views.medications.pharmacists.DailyReviewsContainer', [
      { eventType: app.views.medications.pharmacists.DailyReviewsContainer.EVENT_TYPE_THERAPY_SELECTION_CHANGE }
    ]);
    this.registerEventTypes('app.views.medications.pharmacists.DailyReviewsContainer', [
      { eventType: app.views.medications.pharmacists.DailyReviewsContainer.EVENT_TYPE_CONTENT_CHANGE }
    ]);
    this.registerEventTypes('app.views.medications.pharmacists.DailyReviewsContainer', [
      { eventType: app.views.medications.pharmacists.DailyReviewsContainer.EVENT_TYPE_CONTENT_AUTHORIZED }
    ]);
  ...
  _buildGui: function ()
  {
    ...
    var header = new app.views.medications.pharmacists.DailyReviewsContainerHeader({
      dailyContainer: this,
  ...
  _showConfirmAllTherapiesPlaceholderContainer: function (render)
  {
    ...
    var placeholderContainer = new app.views.medications.pharmacists.ConfirmAllTherapiesPlaceholderContainer({
      view: this.getView(),
  ...
  getReviewContainerInEdit: function ()
  {
    ...
    for (var idx = 0; idx < reviews.length; idx++)
    {
      ...
      if (reviewContainer instanceof app.views.medications.pharmacists.ReviewContainer && reviewContainer.isInEditMode())
      {
  ...
  getDraftReviewsTherapyIds: function ()
  {
    ...
    {
      if (reviewContainer instanceof app.views.medications.pharmacists.ReviewContainer &&
  ...
  addReview: function (reviewData, reorder, editMode)
  {
    ...
    if (this.getShowPlaceHolder() && editMode == true)
    {
      ...
      {
        if (containers[idx] instanceof app.views.medications.pharmacists.ConfirmAllTherapiesPlaceholderContainer)
        {
    ...
    reviewContainer.on(app.views.medications.pharmacists.ReviewContainer.EVENT_TYPE_MODE_SWITCH,
        function (component, componentEvent)
        {
          self.onReviewContainerModeSwitch(component, componentEvent.getEventData().switchedToEdit);
        });
    reviewContainer.on(app.views.medications.pharmacists.ReviewContainer.EVENT_TYPE_THERAPY_REMOVED,
        function (component, componentEvent)
        {
  ...
  {
    var self = this;
    var reviewData = new app.views.medications.pharmacists.dto.PharmacistMedicationReview({
  ...
  onReviewContainerModeSwitch: function (container, toEditMode)
  {
    this.getContentContainer().getComponents().forEach(function (reviewContainer)
    {
      if (reviewContainer instanceof app.views.medications.pharmacists.ReviewContainer && reviewContainer != container)
      {
