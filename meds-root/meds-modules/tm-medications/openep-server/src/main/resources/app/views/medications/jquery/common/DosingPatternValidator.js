Class.define('app.views.medications.common.DosingPatternValidator', 'tm.jquery.Object', {
   ...
   * @returns {tm.jquery.FormField}
   */
  getPatternDurationValidation: function(component, times, duration)
  {
    var self = this;
    return new tm.jquery.FormField({
      component: component,
      ...
      validation: {
        ...
        validators: [
          new tm.jquery.Validator({
            errorMessage: this.getView().getDictionary('interval.between.doses.too.short'),
            isValid: function(times)
            {
              return self.isPatternTimesDifferenceValid(times, duration);
            }
          })]
      }
    });
  ...
  /**
   * @param {Array<Date>}times
   * @param {Number} duration in minutes
   * @returns {boolean}
   */
  isPatternTimesDifferenceValid: function(times, duration)
  {
    for (var i = 0; i < times.length-1; i++)
    {
      var timeDifferenceInMinutes = Math.floor((Math.abs(times[i] - times[i+1])/1000)/60);
