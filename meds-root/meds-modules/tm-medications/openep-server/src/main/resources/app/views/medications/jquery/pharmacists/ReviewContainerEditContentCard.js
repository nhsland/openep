Class.define('app.views.medications.pharmacists.ReviewContainerEditContentCard', 'app.views.medications.pharmacists.ReviewContainerViewContentCard', {
  cls: "edit-card",
  ...
  _problemDescriptionWrapper: null,
  ...
  _buildGUI: function ()
  {
    ...
    var reminderComponent = new app.views.medications.common.VerticallyTitledComponent({
    ...
    var reminderNoteContainer = new app.views.medications.common.VerticallyTitledComponent({
    ...
    var drugRelatedProblemDescContainer = new app.views.medications.pharmacists.ProblemDescriptionEditContainer({
    ...
    var pharmacokineticIissueDescContainer = new app.views.medications.pharmacists.ProblemDescriptionEditContainer({
    ...
    var patientRelatedDescContainer = new app.views.medications.pharmacists.ProblemDescriptionEditContainer({
    ...
    if (this.showSupply)
    {
      this.supplyContainer = new app.views.medications.common.VerticallyTitledComponent({
    ...
    this._problemDescriptionWrapper = new tm.jquery.Container({
      cls: 'problem-description-wrapper',
      layout: tm.jquery.VFlexboxLayout.create("flex-start", "stretch"),
      flex: tm.jquery.flexbox.item.Flex.create(1, 1, "auto")
    });

    this._problemDescriptionWrapper.add(drugRelatedProblemDescContainer);
    this._problemDescriptionWrapper.add(pharmacokineticIissueDescContainer);
    this._problemDescriptionWrapper.add(patientRelatedDescContainer);
    this._problemDescriptionWrapper.add(overallRecommendationContainer);

    this.add(basicDataRow);
    this.add(this._problemDescriptionWrapper);
  ...
  {
    this._loadTherapyDisplayData(
        therapyContainer.getData().getTherapy(),
        result,
        function(displayValue)
        {
          ...
          displayValue.changeType = app.views.medications.TherapyEnums.pharmacistTherapyChangeType.EDIT;

          therapyContainer.setData(new app.views.medications.pharmacists.dto.PharmacistReviewTherapy(displayValue));
          therapyContainer.refresh();
        });
  },
  ...
  _loadTherapyDisplayData: function (originalTherapy, changedTherapy, callback)
  {
    this.getReviewContainer()
        .getView()
        .getRestApi()
        .loadFillPharmacistsTherapyEdit(originalTherapy, changedTherapy, true)
        .then(function onSuccess(therapyWithDisplayValues){
  ...
  addTherapy: function (therapyData)
  {
    ...
    var therapiesListContainer = this.getTherapiesListContainer();
    var presentTherapies = therapiesListContainer.getComponents();
    var therapyCompId = !!therapyData.getTherapy() ? therapyData.getTherapy().getCompositionUid() : null;
  ...
  validateData: function(view)
  {
    ...
    var validationForm = view.getAppFactory().createForm({
    ...
    {
      validationForm.addFormField(this.getDrugRelatedProblemDescContainer().getRecommendationFieldValidation(view));
    }

    if (this.getPharmacokineticIissueDescContainer().hasValues())
    {
      validationForm.addFormField(this.getPharmacokineticIissueDescContainer().getRecommendationFieldValidation(view));
    }

    if (this.getPatientRelatedDescContainer().hasValues())
    {
      validationForm.addFormField(this.getPatientRelatedDescContainer().getRecommendationFieldValidation(view));
    }

    var editorData = this.getEditorData();
    var stoppedOrSuspendedTherapyExists = editorData.getRelatedTherapies().some(function(therapy)
    {
      return therapy.getChangeType() === app.views.medications.TherapyEnums.pharmacistTherapyChangeType.SUSPEND ||
          therapy.getChangeType() === app.views.medications.TherapyEnums.pharmacistTherapyChangeType.ABORT;
    });
    if (stoppedOrSuspendedTherapyExists)
    {
      validationForm.addFormField(new tm.jquery.form.FormField({
        component: this._problemDescriptionWrapper,
        ...
        getValue: function()
        {
          return editorData.getDrugRelatedProblem() ||
              editorData.getPharmacokineticIssue() ||
              editorData.getPatientRelatedProblem() ||
              editorData.getOverallRecommendation();
        }
      }));
    }
  ...
  getEditorData: function()
  {
    ...
    var editorData = new app.views.medications.pharmacists.dto.PharmacistMedicationReview({
    ...
    if (this.getDrugRelatedProblemDescContainer().hasValues())
    {
      editorData.setDrugRelatedProblem(new app.views.medications.pharmacists.dto.TherapyProblemDescription());
      this.getDrugRelatedProblemDescContainer().toTherapyProblemDescription(editorData.getDrugRelatedProblem());
    }

    if (this.getPharmacokineticIissueDescContainer().hasValues())
    {
      editorData.setPharmacokineticIssue(new app.views.medications.pharmacists.dto.TherapyProblemDescription());
      this.getPharmacokineticIissueDescContainer().toTherapyProblemDescription(editorData.getPharmacokineticIssue());
    }

    if (this.getPatientRelatedDescContainer().hasValues())
    {
      editorData.setPatientRelatedProblem(new app.views.medications.pharmacists.dto.TherapyProblemDescription());
