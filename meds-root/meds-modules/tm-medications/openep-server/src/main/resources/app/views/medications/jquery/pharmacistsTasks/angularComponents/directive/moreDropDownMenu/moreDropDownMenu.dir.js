var MoreDropDownMenuDir = function ()
{
  return {
    ...
    controller: ['$scope', '$rootScope', '$timeout', 'TasksOperationService', 'PharmacistsTaskService', 'TaskTypeEnum',
      'tm.angularjs.common.tmcBridge.ViewProxy',
      function ($scope, $rootScope, $timeout, TasksOperationService, PharmacistsTaskService, TaskTypeEnum, viewProxy)
    {
      ...
      $scope.positionDropDownMenu = function ($event)
      {
        ...
        var popupWidth = $scope.dataItem.taskType  === 'SUPPLY_REVIEW' ? 190 : 120;
      ...
      $rootScope.$on('documentClicked', function (event, target, escKey)
      {
        ...
        {
          $timeout(function moreDropDownMenuSafeScopeApply()
          {
            $scope.maskData.showDropDownContextMenu = false;
          });
        }
