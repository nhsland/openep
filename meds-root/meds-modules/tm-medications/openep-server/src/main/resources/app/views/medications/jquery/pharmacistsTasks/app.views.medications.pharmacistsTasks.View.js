Class.define('app.views.medications.pharmacistsTasks.View', 'app.views.common.AppView', {
    ...
    if(this.isDevelopmentMode() === true)
    {
      setTimeout(function()
      {
        self.onViewCommand({
          update: {
            data: {
              //patientIds: [ '399302674', '399302689' ] // optional integration mode, disables the care provider filter
            }
          }
        });
      }, 500);
  ...
  _buildGui: function ()
  {
    ...
    if(this.isMedicationOnPreparationTasklistEnabled())
    {
      ...
      html += '<button type="button" title="' + this.getDictionary('medication.preparation.tasks') + '" data-ng-click="showPerfusionSyringesGrid();" ';
  ...
  updateData: function (command)
  {
    ...
    angular.element(document.getElementById('supplyController')).scope().$emit('updateData', command.data);
  },
