var ThinkGridService = function (viewProxy, OpenPatientClickHandler, ActiveInitData)
{
  ...
  {
    return {
      openPatientDoubleClickHandler : function(rowData, defaultSubview) {
        if (rowData.changeType && ['MEDS_ON_ADMISSION', 'MEDS_ON_DISCHARGE'].contains(rowData.changeType))
        {
          defaultSubview = 'RECONCILIATION';
        }
        OpenPatientClickHandler(rowData.patientDisplayDto.id, defaultSubview);
      },
      rowHeight: 'auto',
      footerHeight: false,
      scrollbarV: false,
  ...
  this.getSupplyGridDefinition = function ()
  {
    ...
    definition.therapySubview = 'TIMELINE';
    var headerHeight = 50;
    ...
    definition.columns = [
      {
        ...
        frozenLeft: false,
        sort: false,
        template: '<meds-patient-banner dto="dataItem"></meds-patient-banner>'
      },
      {
        name: viewProxy.getDictionary('therapy'),
        prop: 'therapyDayDto',
        sort: false,
        template: '<meds-therapy-description dto="dataItem"></meds-therapy-description>',
        height: headerHeight,
        width: widths[1],
  ...
  this.getReviewGridDefinition = function ()
  {
    ...
    var definition = this.getCommonDefinition();
    definition.therapySubview = 'PHARMACIST';
    var headerHeight = 50;
    definition.columns = [
      {
        name: viewProxy.getDictionary('patient'),
