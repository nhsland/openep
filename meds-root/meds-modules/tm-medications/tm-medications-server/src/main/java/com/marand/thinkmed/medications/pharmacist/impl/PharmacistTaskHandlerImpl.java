...
public class PharmacistTaskHandlerImpl implements PharmacistTaskHandler
{
  ...
  {
    final DateTime dueTime = Opt.of(medicationsTasksProvider.loadNextAdministrationTask(patientId, when))
      ...
      final DateTime when)
  {
    ...
    return medicationsOpenEhrDao.findInpatientPrescriptions(patientId, new Interval(hospitalizationStart, Days.THREE))
        .stream()
        .findFirst()
        .map(comp -> new Interval(DataValueUtils.getDateTime(comp.getMedicationOrder().getOrderDetails().getOrderStartDateTime()), Hours.ONE))
        .map(startInterval -> startInterval.contains(when)
                         ? PrescriptionChangeTypeEnum.NEW_ADMISSION_PRESCRIPTION
                         : PrescriptionChangeTypeEnum.ADDITION_TO_EXISTING_PRESCRIPTION)
        .orElse(PrescriptionChangeTypeEnum.ADDITION_TO_EXISTING_PRESCRIPTION);
  }
