...
import com.marand.thinkmed.medications.dto.pharmacist.task.AdmissionReviewTaskDto;
import com.marand.thinkmed.medications.dto.pharmacist.task.DischargeReviewTaskDto;
...
public class PharmacistTaskProviderImpl implements PharmacistTaskProvider
{
  ...
  @Override
  public Opt<String> findPharmacistReviewTaskId(final @NonNull String patientId)
  {
      ...
      final Set<TaskTypeEnum> taskTypes)
  {
    ...
    final List<String> patientIdKeysForTaskTypes = TherapyTaskUtils.getPatientIdKeysForTaskTypes(patientIds, taskTypes);
    //noinspection unchecked
    ...
    final Map<String, PatientDisplayWithLocationDto> patientWithLocationMap = getPatientDataForTasks(tasks);
    return buildPharmacistTasks(tasks, patientWithLocationMap);
  }

  private Map<String, PatientDisplayWithLocationDto> getPatientDataForTasks(final List<TaskDto> tasks)
  {
    final Set<String> patientsWithTasks = tasks.stream()
        .map(t -> (String)t.getVariables().get(MedsTaskDef.PATIENT_ID.getName()))
        .collect(Collectors.toSet());

    return patientDataProvider.getPatientDisplayWithLocationMap(patientsWithTasks);
  }
      ...
      final Map<String, PatientDisplayWithLocationDto> patientIdAndPatientWithLocationMap)
  {
    return tasks
        ...
        .map(t -> {
          ...
          final String strategyId = t.getTaskExecutionStrategyId();

          if (strategyId.equals(TaskTypeEnum.PHARMACIST_REVIEW.getName()))
          {
            return buildPharmacistReviewTask(patientIdAndPatientWithLocationMap, t);
          }
          if (strategyId.equals(TaskTypeEnum.PHARMACIST_REMINDER.getName()))
          {
            return buildPharmacistReminderTask(patientIdAndPatientWithLocationMap, t);
          }
          if (strategyId.equals(TaskTypeEnum.ADMISSION_REVIEW_TASK.getName()))
          {
            return buildAdmissionReviewTask(patientIdAndPatientWithLocationMap, t);
          }
          if (strategyId.equals(TaskTypeEnum.DISCHARGE_REVIEW_TASK.getName()))
          {
            return buildDischargeReviewTask(patientIdAndPatientWithLocationMap, t);
          }
          return null;

        })
        .filter(Objects::nonNull)
        .sorted(patientTaskDtoComparator())
        .collect(Collectors.toList());
  }

  private Comparator<PatientTaskDto> patientTaskDtoComparator()
  {
    return (task1, task2) -> {
  ...
  private AdmissionReviewTaskDto buildAdmissionReviewTask(
      final Map<String, PatientDisplayWithLocationDto> patientIdAndPatientWithLocationMap,
      final TaskDto task)
  {
    final AdmissionReviewTaskDto admissionTask = new AdmissionReviewTaskDto();

    admissionTask.setId(task.getId());

    admissionTask.setLastEditTimestamp(new DateTime(task.getVariables().get(AdmissionReviewTaskDef.LAST_EDIT_TIMESTAMP_MILLIS.getName())));
    admissionTask.setLastEditorName((String)task.getVariables().get(AdmissionReviewTaskDef.LAST_EDITOR_NAME.getName()));

    final String patientId = (String)task.getVariables().get(PharmacistReminderTaskDef.PATIENT_ID.getName());
    fillPatientDataForPatientTaskDto(admissionTask, patientIdAndPatientWithLocationMap, patientId);
    return admissionTask;
  }

  private DischargeReviewTaskDto buildDischargeReviewTask(
      final Map<String, PatientDisplayWithLocationDto> patientIdAndPatientWithLocationMap,
      final TaskDto task)
  {
    final DischargeReviewTaskDto dischargeTask = new DischargeReviewTaskDto();

    dischargeTask.setId(task.getId());

    dischargeTask.setLastEditTimestamp(new DateTime(task.getVariables().get(DischargeReviewTaskDef.LAST_EDIT_TIMESTAMP_MILLIS.getName())));
    dischargeTask.setLastEditorName((String)task.getVariables().get(DischargeReviewTaskDef.LAST_EDITOR_NAME.getName()));

    final String patientId = (String)task.getVariables().get(PharmacistReminderTaskDef.PATIENT_ID.getName());
    fillPatientDataForPatientTaskDto(dischargeTask, patientIdAndPatientWithLocationMap, patientId);
    return dischargeTask;
  }

  private PharmacistReminderTaskDto buildPharmacistReminderTask(
      final Map<String, PatientDisplayWithLocationDto> patientIdAndPatientWithLocationMap,
      final TaskDto task)
  {
  ...
  private PharmacistReviewTaskDto buildPharmacistReviewTask(
      final Map<String, PatientDisplayWithLocationDto> patientIdAndPatientWithLocationMap,
      final TaskDto task)
  {
      ...
      final Locale locale)
  {
    ...
    final Map<String, PatientDisplayWithLocationDto> patientWithLocationMap = getPatientDataForTasks(tasksList);
      ...
      final Locale locale)
  {
    ...
    final Map<String, PatientDisplayWithLocationDto> patientIdAndPatientDisplayDataMap =
        patientDataProvider.getPatientDisplayWithLocationMap(patientIds);
      ...
      final Locale locale)
  {
    ...
    final String userName = RequestUser.getFullName();
    ...
    for (final String originalTherapyId : originalTherapyIdAndPatientIdMap.keySet())
    {
      ...
      final String therapyId = TherapyIdUtils.createTherapyId(originalTherapyId);
      perfusionSyringeLabelDto.setBarCode(BarcodeScannerUtils.convertToNumericRepresentation(therapyId));
