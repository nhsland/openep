...
{
  @Configuration
  public static class FhirEncounterProviderContextTest
  {
    @Bean
    public FhirEncounterProvider fhirEncounterProvider()
    {
      final FhirEncounterProvider fhirEncounterProvider = new FhirEncounterProvider();
      fhirEncounterProvider.setFhirClientFactory(fhirClientFactory());
      fhirEncounterProvider.setFhirProperties(fhirProperties());
      return fhirEncounterProvider;
    }
  ...
  public void testGetEncounter()
  {
    ...
    final EncounterDto encounterDto = fhirEncounterProvider.getEncounter("patient1", "encounter1");
    assertEquals("encounter1", encounterDto.getId());
    assertEquals(EncounterType.INPATIENT, encounterDto.getType());
    assertEquals(EncounterStatus.ACTIVE, encounterDto.getStatus());
    assertEquals("Test Doctor", encounterDto.getDoctor());
  ...
  private void mockFhirClientResult(final Bundle result)
  {
    ...
    Mockito.when(count.and(any())).thenReturn(and);
